(() => {
  ...
  i18n.guc = {
      ...
      "shareTextFallback": "COSTA VIVA. Taya acheküin ajütaa jümüin kasachiki wayuu anaajünüsü süka COSTA VIVA. Paquete completo eesü süka ZIP.",
      ...
      "cancel": "Aakataa",
      "importInvalid": "Archivo ZIP tüü nnojotsü COSTA VIVA anain.",
  ...
  async function buildCommunityPackage(records, options, recipientType, purpose) {
    ...
    const payload = { app: 'COSTA VIVA', schemaVersion: 3, exchangeFormat: 2, exportedAt: createdAt, records: sharedRecords }
    ...
    const filename = `COSTA-VIVA-${createdAt.slice(0,10)}-${packageId.slice(0,8)}.zip`
  ...
  function preparedZipFile(pkg) {
    return new File([pkg.blob], pkg.filename || 'COSTA-VIVA.zip', { type: 'application/zip' })
  }

  function preparedShareFallbackText(pkg) {
    const recipientType = pkg.recipientType || 'other'
    const purpose = pkg.purpose || ''
    const recipient = recipientLabel(recipientType)
    const filename = pkg.filename || 'COSTA-VIVA.zip'
    if (state.lang === 'pt') {
      return `COSTA VIVA\nZIP preparado localmente para compartilhar registros comunitários.\nArquivo ${filename}\nDestinatário ${recipient}${purpose ? `\nFinalidade ${purpose}` : ''}\nSe o ZIP não aparecer anexado automaticamente, ele está salvo em Downloads e pode ser adicionado como documento.`
    }
    if (state.lang === 'guc') {
      return `COSTA VIVA\nZIP anaajünüsü sulu'u celular süpüla ajütaa kasachiki wayuu.\nArchivo ${filename}${purpose ? `\n${purpose}` : ''}\nMüleka nnojotsü ZIP ajütünü jümaa menu, eesü sulu'u Descargas süpüla akeraajaa süka documento.`
    }
    return `COSTA VIVA\nZIP preparado localmente para compartir registros comunitarios.\nArchivo ${filename}\nDestinatario ${recipient}${purpose ? `\nFinalidad ${purpose}` : ''}\nSi el ZIP no aparece adjunto automáticamente, está guardado en Descargas y puede añadirse como documento.`
  }
  ...
  async function openPreparedShareMenu() {
    ...
    const zipFile = preparedZipFile(pkg)

    if (!navigator.share) {
      $('#share-modal-status').textContent = tr('shareUnavailable')
      return
    }

    // No descargamos nada antes de abrir el menú. Una descarga dispara
    // un diálogo del navegador y consume la activación del gesto del usuario,
    // lo que puede impedir que navigator.share() abra la hoja nativa.
    const shareWithZip = { title: 'COSTA VIVA', text, files: [zipFile] }
    const canShareZip = navigator.canShare ? navigator.canShare(shareWithZip) : false

    if (canShareZip) {
      try {
        await navigator.share(shareWithZip)
        $('#share-modal-status').textContent = tr('shareMenuOpened')
        return
      } catch (error) {
        if (error?.name === 'AbortError') return
        console.warn('ZIP share attempt failed', error)
      }
    }

    // Si el navegador no acepta ZIP como archivo compartible, abrimos de todos
    // modos el menú nativo con texto. El ZIP permanece en memoria hasta que
    // la persona pulse Guardar ZIP. Así no aparece ningún diálogo de descarga
    // antes del menú de compartir.
    try {
      await navigator.share({
        title: 'COSTA VIVA',
        text: preparedShareFallbackText(pkg),
        url: window.location.href.split('#')[0]
      })
      $('#share-modal-status').textContent = tr('shareMenuOpenedWithoutZip')
      return
    } catch (error) {
      if (error?.name === 'AbortError') return
      console.warn('Text share fallback failed', error)
      $('#share-modal-status').textContent = tr('shareUnavailable')
    }
  }
  ...
  async function readImportPackage(file) {
    ...
    try {
      ...
      if (!['costa-viva-zip','costa-viva-cvpack'].includes(manifest.format) || ![1,2].includes(manifest.formatVersion) || !Array.isArray(payload.records)) throw new Error('Unsupported package')
    ...
    try {
      for (const raw of payload.records) {
        ...
        const record = {
          ...
          provenance: {
            ...(raw.provenance || {}),
            receivedVia: manifest.format || 'costa-viva-zip'
          },
          exchange: {
            received: true,
            receivedAt,
